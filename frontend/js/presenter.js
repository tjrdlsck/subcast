// CanvasTextBaseline 패치: 브라우저 확장 프로그램 등에서 'alphabetical'을 넣어 발생하는 경고 우회
        ...
        window.onload = () => {
            ...
            const openMonitorBtn = document.getElementById("btn-open-monitoring");
            const closeMonitorBtn = document.getElementById("btn-close-monitoring");
            const saveMonitorBtn = document.getElementById("btn-save-monitoring");
            const openMonitorWindowBtn = document.getElementById("btn-open-monitor-window");

            // 모니터링 실시간 미리보기 업데이트 함수
            function updateMonitorPreview() {
                ...
                const currentSec = document.getElementById("preview-current-sec");
                const nextSec = document.getElementById("preview-next-sec");
                const currentTextEl = document.getElementById("preview-current-text");
                ...
                const curIdx = slides.findIndex(s => s.id === currentLiveId);

                if (curIdx !== -1 && slides[curIdx]) {
                    curText = extractText(slides[curIdx]) || "(빈 슬라이드)";
                    if (curIdx + 1 < slides.length) {
                        nxtText = extractText(slides[curIdx + 1]) || "(빈 슬라이드)";
                    } else {
                        nxtText = "다음 슬라이드가 없습니다 (마지막)";
                    }
                } else if (slides.length > 0) {
                    curText = extractText(slides[0]) || "[요한복음 3:16] 하나님이 세상을 이처럼 사랑하사 독생자를 주셨으니 이는 그를 믿는 자마다 멸망하지 않고 영생을 얻게 하려 하심이라";
                    if (slides.length > 1) {
                        nxtText = extractText(slides[1]) || "[요한복음 3:17] 하나님이 그 아들을 세상에 보내신 것은 세상을 심판하려 하심이 아니요 그를 통하여 세상이 구원을 받게 하려 하심이라";
                    } else {
                        nxtText = "[요한복음 3:17] 하나님이 그 아들을 세상에 보내신 것은 세상을 심판하려 하심이 아니요 그를 통하여 세상이 구원을 받게 하려 하심이라";
                    }
                } else {
                    curText = "[요한복음 3:16] 하나님이 세상을 이처럼 사랑하사 독생자를 주셨으니 이는 그를 믿는 자마다 멸망하지 않고 영생을 얻게 하려 하심이라";
                    nxtText = "[요한복음 3:17] 하나님이 그 아들을 세상에 보내신 것은 세상을 심판하려 하심이 아니요 그를 통하여 세상이 구원을 받게 하려 하심이라";
                }

                function applyBibleMode(text) {
                    if (!text) return "";
                    if (bibleMode === "ref_only") {
                        const match = text.match(/(\[[^\]]+\]|[가-힣]+\s*\d+:\d+)/);
                        if (match) {
                            return `${match[1]} (성경 구절 대기)`;
                        }
            ...
            document.querySelectorAll('input[name="monitor-layout"], input[name="monitor-bible"]').forEach(radio => {
                radio.addEventListener('change', updateMonitorPreview);
            });

            if (openMonitorBtn && monitorModal) {
                openMonitorBtn.onclick = () => {
                    // 저장된 설정을 라디오 버튼에 적용
                    try {
                        const saved = JSON.parse(localStorage.getItem("subcast_monitor_settings") || "{}");
                        if (saved.layout) {
                            const layoutEl = document.querySelector(`input[name="monitor-layout"][value="${saved.layout}"]`);
                            if (layoutEl) layoutEl.checked = true;
                        }
                        if (saved.bibleMode) {
                            const bibleEl = document.querySelector(`input[name="monitor-bible"][value="${saved.bibleMode}"]`);
                            if (bibleEl) bibleEl.checked = true;
                        }
                    } catch (e) { }
            ...
            if (closeMonitorBtn && monitorModal) {
                closeMonitorBtn.onclick = () => monitorModal.classList.remove("active");
            }
            if (monitorModal) {
                monitorModal.onclick = (e) => {
